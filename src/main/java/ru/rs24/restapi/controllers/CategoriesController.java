...
public class CategoriesController {
    ...
    public ResponseEntity<CategoryDto> getCategoryById(@PathVariable Long id) {
        Category category = getCategoriesService().getCategoryById(id).orElseThrow(() -> new EntityNotFoundException("Category not found, id: " + id));
    ...
    public ResponseEntity<CategoryDto> saveCategory(@RequestBody CategoryDto categoryDto) {
        Category category = getCategoriesService().saveCategory(categoryDto.getName(), categoryDto.getShortDescription())
                .orElseThrow(() -> new EntityNotFoundException("Category already exist " + categoryDto.getName()));
        return new ResponseEntity<>(CategoryMapper.INSTANCE.toDto(category), HttpStatus.CREATED);
    }

    @PutMapping
    public ResponseEntity<CategoryDto> updateCategory(@RequestBody CategoryDto categoryDto) {
        Category category = getCategoriesService().updateCategory(categoryDto).orElseThrow(() -> new EntityNotFoundException("Category not found, id: " + categoryDto.getId()));
        return new ResponseEntity<>(CategoryMapper.INSTANCE.toDto(category), HttpStatus.CREATED);
    }
